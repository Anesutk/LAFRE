...
export default function ForumPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!community && !category && communityResult.communities?.length) setCommunity(String(communityResult.communities[0].id));
    } catch (err) {
  ...
  async function submit(event) {
    ...
    if (role === 'visitor') { setError('Sign in as a student to start a discussion.'); return; }
    if (profile?.status !== 'approved') { setError('Your account must be approved before you can post.'); return; }
    if (!title.trim() || !text.trim() || !community) { setError('Title, community, and question are required.'); return; }
    setPosting(true);
    try {
      await apiFetch('/forum/posts/', { method: 'POST', body: JSON.stringify({ title: title.trim(), text: text.trim(), community: Number(community), language: 'en' }) });
      setTitle(''); setText(''); setNotice('Your discussion was posted successfully.');
      await load();
    } catch (err) { setError(err.message || 'Could not post the discussion.'); }
    finally { setPosting(false); }
  }

  async function togglePost(postId, kind) {
    setActionId(`${kind}-${postId}`); setError('');
    try {
      const result = await apiFetch(`/forum/posts/${postId}/${kind}/`, { method: 'POST', body: JSON.stringify({}) });
      setPosts(current => current.map(post => post.id === postId ? {
        ...post,
        ...(kind === 'like' ? { liked: !!result.liked, like_count: result.like_count } : { saved: !!result.saved }),
      } : post));
    } catch (err) { setError(err.message || `Could not ${kind} this post.`); }
