...
function StudentChatContent() {
  ...
  const [profile, setProfile] = useState(null); const [mounted, setMounted] = useState(false); const [sidebarOpen, setSidebarOpen] = useState(false); const [chatId, setChatId] = useState(null); const [chatTitle, setChatTitle] = useState('New chat'); const [chats, setChats] = useState([]); const [chatSearch, setChatSearch] = useState(''); const [messages, setMessages] = useState([]); const [prompt, setPrompt] = useState(''); const [loading, setLoading] = useState(false); const [sourcePanel, setSourcePanel] = useState(null); const [lastPrompt, setLastPrompt] = useState(''); const [mode, setMode] = useState('chat'); const [chatMenuOpen, setChatMenuOpen] = useState(false); const answerStyle = 'balanced'; const bottomRef = useRef(null); const composerRef = useRef(null);
  ...
  // Auto-expand the composer as the user types, capped at a max height with scroll beyond
  // that - the pattern every other AI chat input uses (ChatGPT, Claude, etc). The fixed
  // `rows={1}` textarea before this never grew at all, which is what made typing a longer
  // message feel cramped and non-responsive compared to those.
  function autoResizeComposer(el) {
    if (!el) return;
    el.style.height = 'auto';
    el.style.height = `${Math.min(el.scrollHeight, 160)}px`;
  }
  useEffect(() => { autoResizeComposer(composerRef.current); }, [prompt]);
